...
public class CDVMFPPush extends CordovaPlugin {
    ...
    public boolean execute(String action, JSONArray args, final CallbackContext callbackContext) throws JSONException {
        ...
        if ("initialize".equals(action)) {
            ...
            if (validateString(args.getString(1))){
                String pushUserId = args.getString(1);
                this.registerDeviceWithUserId(pushUserId ,callbackContext);
                return true;
            }
        ...
        cordova.getThreadPool().execute(new Runnable() {
            public void run() {
                if (validateString(pushAppGUID)){
                    ...
                    pushLogger.debug("MFPPush:initialize() - An error occured while initializing MFPPush service. Add a valid  pushAppGUID Value");
                }
            }
    ...
    private void registerDevice(final CallbackContext callbackContext) {
        ...
        if(isInitialized){
            ...
            pushLogger.debug("MFPPush:register() - An error occured while registering for MFPPush service. Push not initialized with call to initialize()");
        }
    }
    
    /**
     * Registers the device for Push notifications with the given userId
     * @param callbackContext Javascript callback
     */
    private void registerDeviceWithUserId(final String pushUserId, final CallbackContext callbackContext) {
        
        if(isInitialized){
            if(validateString(pushUserId)){
                
                cordova.getThreadPool().execute(new Runnable() {
                    public void run() {
                        MFPPush.getInstance().registerDeviceWithUserId(pushUserId, new MFPPushResponseListener<String>() {
                            @Override
                            public void onSuccess(String s) {
                ...
                pushLogger.debug("MFPPush:register() - An error occured while registering for MFPPush service. Add a valid userId Value");
            }
    ...
    private Boolean validateString(String object) {
